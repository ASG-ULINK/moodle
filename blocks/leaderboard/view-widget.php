...
$id = $_GET['id'];
global $DB;
$widget = $DB->get_record_sql("select bc.*,c.id as id, c.* from {block_leaderboard} as bc join {leaderboard} as c on c.id=bc.leaderboardid where bc.id = ".$id);
$widget->widgetcolor = "#fff";
//echo '<pre>widget: ' . print_r($widget, true) . '</pre><br />';
 $leaderboardDetails = $DB->get_record_sql('SELECT * FROM {files} WHERE itemid = ' . $widget->imagefile . ' AND license IS NOT NULL');     
		               ...
		               $fs = get_file_storage();
						...
						$fileinfo = array(
						    ...
						    'contextid' => $leaderboardDetails->contextid, // ID of context
						    'filepath' => '/',           // any path beginning and ending in /
						    'filename' => $leaderboardDetails->filename); // any filename
						...
						if ($file) {
    						$file->copy_content_to("$CFG->dirroot/blocks/leaderboard/logo/");
    						$leaderboardimage = $CFG->wwwroot . "/draftfile.php/$leaderboardDetails->contextid/user/draft/$widget->imagefile/$leaderboardDetails->filename";
						} 
					...
					<table >
						<tr>
							<td style="border:1px solid #d6d6d6;padding:5px;border-radius:5px;background:<?php echo $widget->widgetcolor?>">
								<table>
									<tr>
										<td align="center"><img src = "<?php echo $leaderboardimage ;?>" height="40"></td>
									</tr>
									<tr>
				...
				<td>
					<fieldset style="padding:0px;margin:0px;border-radius:5px;"><legend><a href="javascript:void(0);" style="font-size:12px;font-weight:bold;color:#355435;text-decoration:none" onClick="$('#code').select();">Copy Code</a></legend>
						<table>
							<tr>
								<td>
									<textarea cols="21" rows="7" style="width:215px;border:0px;" id="code"><script language="javascript" type="text/javascript" src="<?php echo $CFG->wwwroot?>/local/leaderboard/leaderboardcatalog.php?id=<?php echo $widget->id?>"></script></textarea>
									</td>
							</tr>				
						</table>
					</fieldset>	
