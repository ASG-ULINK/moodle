...
$PAGE->set_url('/blocks/timeline_plan/ajax_bridge.php');
...
if ($lp_id && $t_id) {
    ...
    // Change training on the base of timeline plan and user.
    if ($lp_id && $u_id) {
        $attributes = $DB->get_records_sql_menu('select lpt.t_id as id, (select training_name from  {timeline_training} where id =lpt.t_id) as name from {timeline_plan_training}
                                                 lpt inner join {timeline_user_trainingplan} lut on lut.lpt_id=lpt.id  where lpt.lp_id=? AND lut.u_id=?', array($lp_id, $u_id), $limitfrom = 0, $limitnum = 0);
    }
    // Find training on the base of lp Id.
    else if ($lp_id && $t) {
        $attributes = $DB->get_records_sql_menu('SELECT t_id as id,(select training_name from  {timeline_training} where id =t_id) as name FROM {timeline_plan_training}
                                                 where lp_id= ?', array($lp_id), $limitfrom = 0, $limitnum = 0);
    }

    // Change users on the base of timeline plan
    else if ($lp_id && $hidetraining) {
        $attributes1 = $DB->get_records_sql_menu('SELECT id, training_name as name from {timeline_training} where type_id=?', array($t_type_id), $limitfrom = 0, $limitnum = 0);
        $attributes2 = $DB->get_records_sql_menu('SELECT  t_id as id, (SELECT training_name from {timeline_training} where id =t_id) as name from {timeline_plan_training}
                                                  where lp_id=?', array($lp_id), $limitfrom = 0, $limitnum = 0);
        ...
        $attributes4 = $DB->get_records_sql_menu('SELECT u_id as id, (SELECT concat(firstname," ", lastname) FROM {user} WHERE id = u_id) as name FROM {timeline_user_timelineplan}
                                                   where lp_id = ?', array($lp_id), $limitfrom = 0, $limitnum = 0);
        ...
        $attributes = $DB->get_records_sql_menu('SELECT u.u_id as id, (SELECT  CONCAT(firstname," ", lastname)FROM {user} where username!="guest" AND id = u. u_id) as name FROM
                                                 {timeline_user_timelineplan} as u where lp_id= ?', array($lp_id), $limitfrom = 0, $limitnum = 0);
    }
    $data = "";
