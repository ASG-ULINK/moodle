...
function xmldb_block_timeline_plan_upgrade($oldversion) {
    global $DB;
    $plugin = new stdClass();
    ...
    if ($oldversion < $plugin->version) {
        ...
        $DB->execute("UPDATE {timeline_user_trainingplan} SET status = '0' WHERE status = 'In-progress'");
        $DB->execute("UPDATE {timeline_user_trainingplan} SET status = '1' WHERE status = 'Not Yet Started'");
        $DB->execute("UPDATE {timeline_user_trainingplan} SET status = '2' WHERE status = 'Complete'");
        // Clean-up unused records in timeline_user_trainingplan that were left behind when training plans were deleted (this won't happen anymore)
        $DB->execute("DELETE FROM {timeline_user_trainingplan} WHERE (SELECT count(1) FROM {timeline_plan_training} WHERE id = {timeline_user_trainingplan}.lpt_id) < 1");
    }
    return true;
}
