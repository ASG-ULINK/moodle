...
$pageurl = '/blocks/timeline_plan/student/view.php?id=' . $lp_id;
...
$PAGE->navbar->add(get_string("pluginname", 'block_timeline_plan'), new moodle_url($pageurl));
echo $OUTPUT->header();
$student_status = display_list($lp_id, $USER->id);
$title = '<table width="100%" style="background-color:#EEE;"><tr><td style="text-align:center;"><h3>'.get_string('status_report', 'block_timeline_plan').'</h3><h3>'.$USER->firstname . ' ' . $USER->lastname .'</h3><h3>'.get_timelineplan_name($lp_id) .'</h3></h3><p>'.get_string('report_at', 'block_timeline_plan') . ' ' .(Date("d M Y")).'</p></td></tr></tr><table>';
echo $title;
