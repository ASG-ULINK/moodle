...
<div id="page" class="container-fluid mycontainer<?php //echo "$setfull"; ?>">
...
.head-text h3 {
color: #fff !important;
padiing: 5px 10px 0 20px!important;
font-size: 17px;
...
background-image: url('<?php p($CFG->wwwroot) ?>/theme/bcu/layout/includes/images/header.jpg') !important;
...
.navbar { background: #2E3A3F !important;
    margin-top:-40px; 
}
.navbar .nav>li>a {
    color: #fff !important;
...
#above-header { border-bottom: none !important;}
.navbar {margin-left: 60px;}

.get-btn {
/*background: #ECDADA!important;*/
background-image: url('<?php p($CFG->wwwroot) ?>/theme/bcu/layout/includes/images/button-getting-started.png') !important;
padding: 5px;
float: left;
border-radius: 4px;
/*margin-left: 5%;*/
...
<li><a href="<?php p($CFG->wwwroot) ?>/login/logout.php" title="Log out"><i class="fa fa-lock"></i><?php echo get_string('logout') ?></a></li>
   ...
   <?php } ?>
    ...
    <header id="page-header" class="clearfix mycontainer">
       <!--  <?php if ($haslogo) { ?>
...
<div class="head-text">
    <h3>UNIVERSAL LEARNING INTEGRATED NETWORK CONNECTION</h3>
    </div>